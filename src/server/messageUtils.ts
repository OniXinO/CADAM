import type { CoreMessage } from '@shared/types';
...
export async function formatCreativeUserMessage(
  ...
  if (message.content.mesh) {
    ...
    parts.push({
      type: 'text',
      text: previewSignedUrl
        ? `Here is a preview of the mesh with the ID ${message.content.mesh.id}`
        : `User uploaded mesh with the ID ${message.content.mesh.id}`,
    });
    if (previewSignedUrl) {
      parts.push({
        type: 'image',
        source: { type: 'url', url: previewSignedUrl },
      });
    }
  }
